...
from src.spatial_vx.utils import get_attributes_for_feat, remove_key_from_list
from src.spatial_vx.feature_comps import feature_comps
from src.spatial_vx.feature_axis import feature_axis


def interester(x, properties=None,
               weights=None, b1=None,
               b2=None, verbose=False, *params):
    if b2 is None:
        b2 = npy.array([100, 90, 0.8, 0.25, 85, 400, 200, 200, 400, 0.25, 200])
    if b1 is None:
        b1 = npy.array([35, 30, 0, 0, 0.5, 35, 20, 40, 120, 1, 40])
    if weights is None:
        weights = npy.array([0.24, 0.12, 0.17, 0.12, 0, 0, 0, 0,
                             0, 0, 0.35])
    ...
    if (x['Xlabelsfeature'] is None or 'labels_1' not in x['Xlabelsfeature'].keys()) or \
            (x['Ylabelsfeature'] is None or 'labels_1' not in x['Ylabelsfeature'].keys()):
    ...
    def ifun(id, bigX, Xhat, b1, b2, a0, a1, p, *params):
        ...
        A = feature_comps(Ytmp, Xtmp, which_comps=p)
        nomen = A.keys()
    ...
    N = len(x['Xlabelsfeature'].keys()) - 9
    M = len(x['Ylabelsfeature'].keys()) - 9
    ...
    res1 = npy.apply_along_axis(ifun, axis=1, arr=ind, bigX=x['Xlabelsfeature'], Xhat=x['Ylabelsfeature'],
...
if __name__ == '__main__':
    ...
    #data = npy.load(r'F:\Work\MODE\tra_test\FeatureFinder\deltammResult_PA3.npy', allow_pickle=True).tolist()
    data = look_deltamm.copy()
    look_interester = interester(data)
    print("hello")
