...
def restructuring(ob_id_list,fo_id_list):
    ...
    for i in range(n_row):
        if ob_id_list[i] in ob_index_dict.keys():
            ...
            index = index + 1
        index_list_dict[index1].append([ob_id_list[i],fo_id_list[i]])
...
def centmatch(x, criteria, const, distfun, areafac, verbose):
    ...
    X = x['Xlabelsfeature']
    Y = data_pre_PA1.pick_labels(Y)
    X = data_pre_PA1.pick_labels(X)
    m = len(Y)
    n = len(X)
    if (criteria != 3):
        Ax = np.zeros((n))
    ...
    for i in range(m) :
        ...
        if (criteria != 3):
            ...
            tmpy_i = featureProps(x = out['Yprop'][i], whichprops = ["centroid","area"])
            tmpy_num = {'{}'.format(i):tmpy_i}
            tmpy.update(tmpy_num)
            Ay_i = math.sqrt(tmpy[str(i)]['area'])
            Ay = np.append(Ay, Ay_i)
        else :
            tmpy = featureProps(x = out['Yprop'][i], whichprops = ["centroid"])
        ...
        for j in range(n):
            if (verbose):
                print(j)
            if (criteria != 3):
                tmpx_j = featureProps(x = out['Xprop'][j], whichprops = ["centroid","area"])
                tmpx_num = {'{}'.format(j):tmpx_j}
                tmpx.update(tmpx_num)
                Ax_j = math.sqrt(tmpx[str(j)]['area'])
                Ax = np.append(Ax, Ax_j)
            else:
                tmpx = featureProps(x = out['Xprop'][j], whichprops = ["centroid"])
    ...
    if len(a_fcst) != m :
        unmatched_X = set(np.arange(m)) - set(a_fcst)    #X未匹配的目标
        #print(unmatched_X)    
    else :
        unmatched_X = 'NULL'
    if len(a_obs) != n :
        unmatched_Xhat = set(np.arange(n)) - set(a_obs)    #Xhat未匹配的目标
...
look_centmatch = centmatch(x = look_FeatureFinder.copy(), criteria = 1, const = 14, distfun = "rdist", areafac = 1, verbose = False)
